...
- Новую команду печати **"Счет-оферта (Конвейер 1С)"** в документе счета покупателю.
...
- MXL-макет, который можно доработать под стиль компании.
- Опциональную подпись `Создано на Конвейере 1С - https://gitsell.ru/1c`.
...
Печатная форма проверена через тестовый HTTP bridge в демо-базе `fresh-bp-demo`.
